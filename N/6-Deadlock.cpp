...
int Allocation[10][2], Max[10][2], Available[2], Need[10][2], c_Available[2];
...
int main()
{
  ...
  if (result == 1)
  {
    ...
    {
      cout << endl
           << "Press 1 for resource allocation: ";
      cout << endl
      ...
      else
      {
        ...
        if (req_need())
        {
          ...
          {
            cout << endl
                 << "Request greater than allocation";
          }
        }
        else
        {
...
int safety_check()
{
  ...
  while (pcom < n)
  {
    ...
    {
      cout << endl
           << "Deadlock found!!! It's Unsafe here";
      return 0;
    }
  }
  cout << endl
...
void resource_request()
{
  ...
  while (pcom < n)
  {
    ...
    {
      cout << endl
           << "Cannot allocate requested resource!!!";
      for (j = 0; j < 2; j++)
      {
        c_Available[j] = c_Available[j] + Request[rp][j];
        Allocation[rp][j] = Allocation[rp][j] - Request[rp][j];
